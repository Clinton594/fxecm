...
function formInitialize(p, t, key = 1) {
  ...
  $("#" + a)
    ...
    .click(function () {
      if ($(this).prop("checked"))
        $(this).parent().parent().find("input:checkbox").prop({
          checked: true,
        });
      else
        $(this).parent().parent().find("input:checkbox").prop({
  ...
  $("#" + a + " .rlAll").click(function () {
    var cd = $(this).val();
    if ($(this).prop("checked")) $("#" + a + " .cbr" + cd).prop({ checked: true });
    else $("#" + a + " .cbr" + cd).prop({ checked: false });
...
function loadOpen(a) {
  ...
  var condition = cnd.join("|");
  if (search_cols.length && search != "") search_cols = `&search_cols=${search_cols.join(",")}`;
  else search_cols = "";
  ...
  $.post(param, function (res) {
    ...
    if (result.row == undefined || result.row.length == 0) {
      ...
      if ($("#_searchBox" + a).val() == "")
        eText =
          "No " +
          pagetitle +
          ` found to display ${range}, click the ( <i class="material-icons">more_vert</i> ) icon above to create new ${pagetitle}`;
        ...
        $("<tr>").append(
          $("<th>").append(
            $("<label>")
              .append(
                $("<input>")
                  .attr({ type: "checkbox", id: "_selectAll" + a })
                  .addClass("cbx_all filled-in")
                  .click(function (e) {
                    e.stopPropagation();
    ...
    $.each(result.row, function (i, field) {
      ...
      $("<label>")
        ...
        .append(
          $("<input>")
            .addClass("form-check-input cbx filled-in")
            .attr({
              type: "checkbox",
              id: "cbx_" + field.i + a,
              value: field.i,
...
function loadDetailTab(ths, a) {
  ...
  let process_url = `${site.process}list?pageType=${tab_data.source}&p=0&l=4&limit=100&condition=${condition.attr(
    "name"
  )},${condition.val()}`;
...
function loadTopLevel(ths, a) {
  ...
  var colvalue = $(ths).data("id");
  if (ths.active == 1) return 0;
  var colname = $("#hierachy" + a).val();
  $("loader").addClass("hide");
  ths.active = 0;
  $("#" + a).swapDiv("#" + parentTitle);
  $("#_linkInput" + parentTitle).val(colvalue);
  var n = $("#_linkFilter" + parentTitle).attr("name");
  $("#_linkFilter" + parentTitle).val(colname + "," + colvalue);
  $("#_linkTitle" + parentTitle).html($(ths).attr("data-title"));
  //alert(parentTitle);
  loadOpen(parentTitle);
  $("#" + parentTitle)
    .find("form")
    .each(function () {
      if ($(this).find("input[name=" + colname + "]")[0] == null) {
        $(this).append($("<input>").attr({ type: "hidden", name: colname, value: colvalue }));
      } else {
        $(this)
          .find("input[name=" + colname + "]")
          .val(colvalue);
      }
...
function performSave(a, ths, callback) {
  ...
  if ($("#trans_no" + a).val() != undefined) {
    ...
    let p = typeof callback == "number" ? "?print=1" : "";
    process_url = `${site.process}transaction${p}`;
  }
      ...
      $.post(
        ...
        function (data) {
          ...
          if (data == "1") {
            ...
            M.toast({
              html: "Error deleting rows or you donot have access for deletion",
              displayLength: 4000,
              classes: "red",
            });
        }
...
$.fn.extend({
  prepareMultiPagerForm: function (a) {
    ...
    if (thsfom.find(".page").length > 1) {
      // If pages are more than one
      thsfom.find(".page").not(".active").hide();
      thsfom.find(".flt ul").prepend(
        $("<li>")
          .hide()
          .append(
            $("<a>")
              .addClass("prevPage")
              .append($("<img>").attr({ src: "icons/chevron_left.svg" }).addClass("material-icons small"))
          )
      );
      ...
      thsfom
        .find(".flt button.formSave")
        .removeAttr("id")
        .addClass("nextPage teal")
        .removeClass("red")
        .off("click")
        .click(function () {
          var form = $(this).closest("form");
      ...
      thsfom.find(".flt ul").append(
        $("<li>")
          .hide()
          .append(
            $("<button>")
              .attr({ id: "formSave" + a })
              .addClass("btn-floating orange")
              .click(function () {
                saveForm(a, this);
              })
...
function reload_selects(a) {
  ...
  setTimeout(function () {
    ...
    $("#formData" + a)
      .find("select[data-target]")
      .each(function () {
        arr.push($(this).data("target") + a);
      });
    $("#formData" + a)
      .find("select:not(.keep)")
      .each(function () {
