...
$.fn.extend({
  emptyState: function (message) {
    var $this = $(this);
    var ehd = $("<div>")
      .addClass("center empty")
      .appendTo($this)
      .css({ width: "100%", padding: "5%" })
    ...
    $("<span>")
      ...
      .addClass(" grey-text")
      .appendTo(ehd)
      .css({ "font-size": "1.5rem", margin: "10%" });
    $(ehd).show();
    return ehd;
  },
});
...
$.fn.extend({
  checkNumeric: function () {
    $(this).keydown(function (e) {
      ...
      if (rst == -1) {
        if (num2.length == 1) e.preventDefault();
      }
      if (rst2 != -1 && num2 == ".") {
        e.preventDefault();
      }
...
$.fn.extend({
  createHeaderRow: function (field, order, extension) {
    var data = hidden(order),
      _hidden = data.hidden,
      total = data.count,
      _notHidden = parseInt(total) - parseInt(_hidden);
    var thead = $(this);
...
$.fn.extend({
  createRow: function (field, order, ext, classs) {
    ...
    if (ext.length) {
      $("<td>")
        .addClass("extcol d-flex justify-content-between")
        .appendTo($(this))
        .optionBox(ext, field.i);
    }
  },
});
  ...
  optionBox: function (field, id) {
    if (field != null && field != undefined) {
      var vt = $("<a>")
        ...
        .append(
          $("<i>")
            .addClass("material-icons col s1 right")
            .append($("<img>").attr({ src: "icons/more_vert.svg" }))
        );
      ...
      $.each(field, function (i, col) {
        ...
        var ed2 = $("<button>")
          .addClass("btn btn-outline-primary btn-sm round")
          .attr({
            "data-formid": col.formId,
            title: col.page_title,
            "data-pageid": col.pageid,
            "data-id": id,
            "data-open": opener,
          })
          .append(
            $("<span>")
              .addClass("editors left w-100")
              .append(
                $("<i>")
                  .addClass(`fa fa-${col.icon} text-white`)
                  .css({ margin: "4px 0px 0px 1px" })
              )
          )
          .click(function (e) {
            e.stopPropagation();
        ...
        if (i < 3) $(ed2).appendTo($(thx));
          ...
          $(ed2)
            .appendTo($(bDiv))
            .addClass("row")
            .append($("<p>").addClass("uc_arng").html(col.text));
          $(vt).appendTo($(thx));
        }
      });
...
$.fn.extend({
  uploadOptn: function (c, f) {
    ...
    var uc_optn = $("<div>")
      .addClass("uploadOptn")
      .css({ position: "absolute", top: "8px", right: "5px", "z-index": "1" })
      .appendTo(nDiv)
      .click(function () {
        $(this).parent().children(".ed_del").show(300);
      });
    var uc_i = $("<img>")
      .addClass("material-icons grey-text text-darken-1")
      .attr({ src: "icons/more_vert_black.svg" })
      .appendTo(uc_optn);
      ...
      .click(function () {
        ...
        edit.keyup(function (e) {
          if (e.keyCode == 13) {
            ...
            if (type == undefined) var new_pth = inc_pth + new_name;
            else var new_pth = inc_pth + new_name + ext;
            var urlsend = $("#upload_modal").attr("data-href");
            $("#upload_modal .progress").removeClass("hide");

            $.post(
              urlsend,
              { case: "renameFile", newPath: new_pth, oldPath: old },
              function (response) {
    ...
    var ed3 = $("<span>")
      ...
      .click(function () {
        ...
        var type = ref.attr("data-type");
        if (type == undefined) typ = 0;
        else typ = 1;
        $(this).questionBox("Are you sure you want to delete", function () {
          $("#upload_modal .progress").removeClass("hide");
          $.post(
            urlsend,
            { type: typ, delPath: src, case: "deleteFiles" },
            function (response) {
              $("#upload_modal .progress").addClass("hide");
              let res = isJson(response);
              if (res.status) {
                ...
                M.toast({
                  html: "An error occured, check console",
                  displayLength: 2000,
                  classes: "red",
                });
              }
    ...
    $.each(field.c, function (j, col) {
      ...
      if (order[j] == "t") {
        ...
        nDiv.append(
          $("<span>")
            .text(col + " ")
            .css({ "font-size": "11px", "font-style": "italic" })
        );
      } else if (order[j] == "rightBold") {
        nDiv.append(
          $("<span></span>")
            .text(col + " ")
            .css({ float: "right", "font-weight": "bold", "font-size": "12px" })
        );
      } else if (order[j] == "d") {
        ...
        nDiv.append(
          $("<h6>")
            .css({ "margin-top": "0px", "font-weight": "bold" })
            .html(col + " ")
        );
      } else if (order[j] == "a") {
        nDiv.append(col + " ");
...
$.fn.extend({
  createRowList: function (field, order, a) {
    var nD = $("<a>")
      ...
      .appendTo($(this))
      .attr({ id: field.i + a, "data-id": field.i, href: "javascript:;" });
...
$.fn.extend({
  appear: function () {
    $(this)
      .css({ "margin-top": "10px", opacity: 0 })
      .animate({ "margin-top": "0px", opacity: 100 }, "fast");
  },
});
$.fn.extend({
  swap: function (a) {
    $(this).animate({ "margin-left": "-5px", opacity: 0 }, "fast", function () {
      $(this).hide();
      $(a)
        .css({ "margin-left": "5px", opacity: 0 })
        .show()
        .animate({ "margin-left": "0px", opacity: 100 }, "fast");
    });
  },
});
    ...
    swapDiv: function (toshow, callback) {
      if ($(this).length) {
        ...
        $(this).animate({ marginLeft: "-5%", opacity: 0 }, "fast", function () {
          ...
          $(toshow)
            .show()
            .css({ opacity: 0, "margin-left": "-5%" })
            .animate({ marginLeft: "0%", opacity: 1 }, "slow");
...
$.fn.extend({
  divPrint: function (q, f, h) {
    ...
    if ($("#printID")[0] != null) var p = $("#printID");
      ...
      var p = $("<iframe>")
        .attr({ height: "0px", width: "0px", id: "printID" })
        .hide()
        .appendTo($(this));
    ...
    if (windowToWriteTo.document) documentToWriteTo = windowToWriteTo.document;
    iframe = document.frames
      ? document.frames["printID"]
...
$.fn.extend({
  printJournal: function (q, f, h) {
    var header =
      "<html><head><link href='css/materialize.css' type='text/css' rel='stylesheet'><link href='css/print.css' type='text/css' rel='stylesheet'></head><body class='white'>";
    var footer = "</body></html>";
    var title = $("<div>").html($("#_logo").html()).addClass("row  rhead");
    var sb1 = $("<div>").html($("#_address").html()).addClass("cpaddress");
    var sb2 = $("<h5>")
      .html($(this).attr("data-type").replace(/_/g, " "))
      .addClass("col s12 tname rowline");
    ...
    var ft = $("<div>")
      .addClass("row capitalize foot")
      .append(
        $("<div>")
          .addClass("col s3 offset-s1 brow ")
          .html("Processed By: " + getCookie("ELIMS-Login_Name"))
      )
      .append(
        $("<div>")
          .addClass("col s3 offset-s1 bline")
          .html("For:" + $("#_name").html())
      );
    $(v).append(ft);
    var page = header + v.html() + footer;
...
$.fn.extend({
  printDiv: function (q, f, h) {
    ...
    $(v)
      .find(".words")
      .html("Amount in words: " + ConvertWord(total) + " Naira Only");
    $(v).find(".prepayment").parent().hide();
    var prc =
      parseInt($(v).find(".count").html()) +
      parseInt($(v).find(".invoice_count").html());
    $(v)
      .find(".table")
      .each(function (i, j) {
        //if($(this).is(':visible'))var tr=$(this).turnTable(prc);
    ...
    var ft = $("<div>")
      .addClass("row capitalize foot")
      .append($("<div>").addClass("col s3  bline").html("Customer Sign"))
      .append(
        $("<div>")
          .addClass("col s3 offset-s1 brow ")
          .html("Processed By: " + getCookie("ELIMS-Login_Name"))
...
$.fn.extend({
  turnTable: function (prc) {
    ...
    var cr = 0;
    for (
      var v = 0, ch = $(this)[0].attributes, l = $(this)[0].attributes.length;
      v < l;
      v++
    ) {
    ...
    $(this)
      .children()
      .each(function (i) {
        if (!i) n1 = $("<thead>").appendTo($(n));
        else n1 = $("<tr>").appendTo($(n));
              ...
              $(this)
                ...
                .each(function () {
                  ...
                  if ($(this).css("display") !== "none" && k !== "a") {
                    ...
                    for (
                      var v = 0,
                        ch = $(this)[0].attributes,
                        l = $(this)[0].attributes.length;
                      v < l;
                      v++
                    ) {
...
function render(a, b) {
  ...
  for (
    var i = 0, ch = $(a)[0].attributes, l = $(a)[0].attributes.length;
    i < l;
    i++
  ) {
...
function editPicture(a, b) {
  editModal(a, function (o, c) {
    ...
    $.post("process_generic.php", c)
      .done(function (rspText, status, xHr) {
        //$('#'+dv).createPictureCard(c);
        alert(rspText);
      })
      .fail(function (e) {
        M.toast({
          html: "Error in sending message",
          displayLength: 1000,
          classes: "",
        });
      });
...
$.fn.extend({
  myDropOptions: function (v) {
    ...
    var uc_optn = $("<div>")
      .addClass("uploadOptn")
      .css({ position: "absolute", top: "5px", right: "5px", "z-index": "10" })
      .appendTo(nDiv)
      .click(function () {
        $(this).parent().children(".ed_del").show(300);
      });
    var uc_i = $("<img>")
      .attr({ src: "icons/more_vert_black.svg" })
      .addClass("material-icons indigo-text text-darken-1 pointer")
      .appendTo(uc_optn);
...
$.fn.extend({
  dropDownInput: function (x, s, f) {
    ...
    var re_input = $("<input>")
      .addClass("input3")
      .attr({ type: "text", id: "search_pic" + id })
      .keyup(f)
      .appendTo(refresh_inp);
    $(this).click(function () {
...
$.fn.extend({
  ucEmpty2: function (a) {
    ...
    var dv = $("<div>")
      .addClass("row uc_empty")
      .css({ width: "80%" })
      .appendTo(this);
    var icon = $("<img>")
      .addClass("material-icons purple-text")
      .css({ "font-size": "200px", margin: "50px 0" })
      .attr({ src: "icons/" + a.icon + ".svg" })
      .appendTo(dv);
    var stmt = $("<h4>")
      .html(a.first_text)
      .css({ "margin-top": "50px" })
...
$.fn.extend({
  ucEmpty: function (c, f, btn_type, t) {
    ...
    var icon = $("<img>")
      .addClass("material-icons pink-text")
      .attr({ src: "icons/hourglass_empty.svg" })
      .appendTo(dv);
    ...
    if (btn_type == 1) {
      ...
      var anc = $("<a>")
        .attr({ href: "javascript:;" })
        .css({ "text-transform": "unset", "margin-top": "10px" })
...
$.fn.extend({
  ucOptions: function (s, t, u, v) {
    if (s == undefined || s == 0) {
      ...
      var mu = $("<input>")
        .attr({ type: "checkbox" })
        .on("change", course_tree.changePublish);
      //if(t==null )alert('null')
      if (t == undefined || t == 0 || t == null) var g = 0;
      else $(mu).attr({ checked: "" });
      ...
      var ii = $("<img>")
        .addClass("material-icons  waves-effect uc_vert")
        .attr("onclick", "course_tree.showOptions(this)")
        .attr({ src: "icons/more_vert_black.svg" })
        .appendTo(span1);
      ...
      switch (u) {
        ...
        default:
          var edit = $("<span>")
            .addClass("row")
            .attr({ onclick: "course_tree.edit_course(this)" });
          break;
      }
      ...
      var del_icon = $("<img>")
        .addClass("material-icons grey-text left")
        .attr({ src: "icons/delete_black" })
        .html("");
      var del_p = $("<p>").addClass("uc_arng").html("Delete");
      var del = $("<span>")
        .addClass("row")
        .attr({ onclick: "course_tree.cancel_createFolder(this)" })
        .append(del_icon)
        .append(del_p);
      ...
      var eddel1 = $("<img>")
        .addClass("material-icons waves-effect no_vert_align")
        .attr({ onclick: "course_tree.showOptions(this)" })
        .attr({ src: "icons/more_vert_black.svg" });
      var ed_del = $("<span>")
        .append(eddel1)
      ...
      var ed3p = $("<p>")
        ...
        .html("Delete")
        .appendTo(ed3)
        .on("click", course_tree.folder_remove);
...
$.fn.extend({
  createFolderRow: function (a) {
    var nf = $("<ul>")
      .slideDown(300)
      .addClass("collapsible margLeft10 arrange_icon")
      .attr({
    ...
    var sp1 = $("<span>")
      .html(a.name)
      .attr({ onclick: "course_tree.openclose(this)" })
      .addClass("open-cloze");
    var nf2 = $("<a>")
      .append(sp1)
      .addClass("collapsible-header waves-effect my_trans  active")
      .appendTo(nf1);
    var nf3 = $("<img>")
      .attr({ src: "icons/folder_open.svg" })
      .addClass("material-icons tiny foldam ")
      .attr({ onclick: "course_tree.openclose(this)" })
      .prependTo(nf2);
    var nf4 = $("<div>").addClass("collapsible-body").appendTo(nf1);
    var ul1 = $("<ul>").appendTo(nf4);
    $("<img>")
      .attr({ src: "icons/add.svg" })
      .attr({ onclick: "course_tree.createFolder(this)", "data-id": "" })
      .addClass("material-icons blue-text pointer add_icon ")
      .prependTo($(nf1));
    $(nf1).ucOptions(1);
    return nf;
  },
...
$.fn.extend({
  createTOC: function (a) {
    var nf = $("<ul>")
      .addClass("collapsible margLeft10 arrange_icon")
      .attr({ "data-id": a.id, id: "c" + a.id })
      .appendTo($(this));
    var nf1 = $("<li>")
      .attr({
        "data-id": a.id,
        "data-name": a.name,
        "data-creatn": a.creation_type,
        "data-type": a.type,
        "data-src": a.linkd,
        "data-path": a.pathl,
      })
    ...
    var sp1 = $("<span>")
      .html(a.name)
      .attr({ title: a.name, class: "truncate" });
    var nf2 = $("<a>")
      .append(sp1)
      .addClass("collapsible-header waves-effect my_trans ")
      .appendTo(nf1); //.setIcon({'icon':a.creation_type});
    var after = a.creation_type == "file" ? "no_after" : "";
    $("<i>")
      .html(course_tree.setIcon({ format: a.creation_type, icon: a.type }))
      .addClass("material-icons tiny foldam " + after)
      .prependTo(nf2);
    var nf4 = $("<div>").addClass("collapsible-body").appendTo(nf1);
...
$.fn.extend({
  createFolderContents: function (a) {
    ...
    var no_red = $("<a>")
      .addClass("no_redirect redy")
      .html(a.name)
      .attr({ href: "javascript:;", "data-id": a.id })
      .off("click", course_tree.sub_contents_handler)
      .on("click", course_tree.sub_contents_handler);
    var inc = $("<i>")
      .addClass("material-icons circle " + colos[colo])
      .html(course_tree.setIcon({ format: a.creation_type, icon: a.type }));
    var li = $("<li>")
      .appendTo($(this))
      .attr({
  ...
  createNav: function (f, p) {
    ...
    $.each(f, function (i, v) {
      if (i > 0)
        var inc = $("<span>").append(
          $("<img>")
            .attr({ src: "icons/navigate_next.svg" })
            .addClass("material-icons")
        );
      var anc = $("<a>")
        .addClass("no_shift grey-text text-darken-4")
        .html(v.name)
        .attr({ "data-id": v.id, href: "javascript:;", "data-path": v.path2 });
      var cosp = $("<div>")
        .appendTo($(nDiv))
        .addClass("cos_p1")
        .append(inc)
  ...
  createContents: function (f, p) {
    ...
    $.each(f, function (i, v) {
      ...
      var no_red = $("<a>")
        .addClass("no_redirect redy")
        .html(v.name)
        .attr({ href: "javascript:;", "data-id": v.id, "data-path": v.pathl });
      ...
      if (v.type) {
        ...
        switch (v.type) {
          ...
          case "html":
            icn = "subject";
            $(no_red).removeAttr("onclick").on("click", createRichtext);
            break; //http
          case "picture":
            icn = "";
            ...
            break;
          case "assign":
            icn = "assignment_returned";
            $(no_red).removeAttr("onclick").on("click", createAssign);
            break;
          case "quiz":
      ...
      var li = $("<li>")
        ...
        .attr({
          "data-id": v.id,
          "data-creatn": v.creation_type,
          "data-type": v.type,
          "data-src": v.linkd,
          "data-path": v.pathl,
          "data-assignType": v.assign_type,
          "data-skippable": v.skippable,
        })
        .addClass("collection-item avatar")
        .append(inc)
        .append(no_red);
...
$.fn.extend({
  loader2: function (a) {
    ...
    var loader = $("<div>")
      ...
      .append(
        $("<div>")
          .addClass("spinner-layer spinner-red-only")
          .append(
            $("<div>")
              .addClass("circle-clipper left")
              .append($("<div>").addClass("circle"))
    ...
    var abs = $("<div>")
      .css({ right: r, top: t, "z-index": 10010 })
      .addClass("fixed lms_loader2")
      .append(loader);
...
$.fn.extend({
  completeSwap: function (a) {
    ...
    $(a)
      .show()
      .css({ left: "120%" })
      .animate({ left: "0%" }, 800, "linear", function () {
        $(a).css({ position: "relative" });
      });
    $(this).animate({ left: "-120%" }, 800, "linear", function () {
...
$.fn.extend({
  registrationSwap: function (a, d) {
    ...
    var transit = {
      "-moz-transition":
        "-moz-transform 0.8s linear 0s, opacity 0.8s linear 0s",
      "-ms-transition": "-ms-transform 0.8s linear 0s, opacity 0.8s linear 0s",
      "-webkit-transition":
        "-webkit-transform 0.8s linear 0s, opacity 0.8s linear 0s",
...
$.fn.extend({
  questionBox: function (q, callback, mini = false) {
    if ($("#_question_dialog").get(0) == null) {
      var div = $("<div>")
        .addClass("modal fade")
        .attr({ role: "dialog", id: "_question_dialog" })
      ...
      $("<img>")
        .attr({ src: "icons/warning.svg" })
        .addClass("large material-icons left")
        .appendTo(cntr)
        .css({ margin: "20px" });
      $("<div>")
        .appendTo(cntr)
        .attr({ id: "_question_text" })
        .css({ "margin-top": "16px", "font-size": "16px" });
      var mod_ft = $("<div>").addClass("modal-footer").appendTo(cotent);
      var mod_fta = $("<a>")
        .addClass("waves-effect waves-blue bg-primary btn text-white btn-flat")
        .attr({ href: "javascript:;", id: "_question_action" })
        .html("OK")
        .appendTo(mod_ft);
      var mod_ftax = $("<a>")
        .addClass("waves-effect waves-blue btn-flat")
        .attr({ href: "javascript:;", "data-dismiss": "modal" })
        .html("CANCEL")
        .appendTo(mod_ft);
      div.on("shown.bs.modal", function () {
        $(mod_fta).click(function () {
          $("#_question_dialog").closeModal();
        });
        $(mod_ftax).click(function () {
          $("#_question_dialog").closeModal();
        });
      });
    }
...
$.fn.extend({
  printDiv: function (q, f, h) {
    ...
    var ft = $("<div>")
      .addClass("row")
      .append(
        $("<div>").addClass("col s4 offset-s1 bline").html("Customer Sign")
      )
      .append($("<div>").addClass("col s4 offset-s1 bline").html("Sign/stamp"));
    $(v).append(ft);
    var page = header + v.html() + footer;
    if ($("#printID")[0] != null) var p = $("#printID");
    else {
